# QBCore Framework Reference Links

## Official Documentation

- **Main Documentation**: <https://docs.qbcore.org>
- **GitHub Repository**: <https://github.com/qbcore-framework>
- **Discord Community**: <https://discord.gg/qbcore>

## Core Documentation

### Getting Started

- **QBCore Documentation**: <https://docs.qbcore.org/qbcore-documentation>
- **QB-Core Object**: <https://docs.qbcore.org/qbcore-documentation/qb-core/core-object>
- **Script Optimization**: <https://docs.qbcore.org/qbcore-documentation/guides/script-optimization>

### Client-Side

- **Client Function Reference**: <https://docs.qbcore.org/qbcore-documentation/qb-core/client-function-reference>
- **Client Event Reference**: <https://docs.qbcore.org/qbcore-documentation/qb-core/client-event-reference>

### Server-Side

- **Server Function Reference**: <https://docs.qbcore.org/qbcore-documentation/qb-core/server-function-reference>
- **Server Event Reference**: <https://docs.qbcore.org/qbcore-documentation/qb-core/server-event-reference>
- **Player Data**: <https://docs.qbcore.org/qbcore-documentation/qb-core/player-data>

### Configuration

- **Shared Configuration**: <https://docs.qbcore.org/qbcore-documentation/qb-core/shared-configuration>
- **Commands**: <https://docs.qbcore.org/qbcore-documentation/qb-core/commands>

## QBCore Resources

### Essential Resources

- **qb-core**: <https://github.com/qbcore-framework/qb-core>
- **qb-multicharacter**: <https://github.com/qbcore-framework/qb-multicharacter>
- **qb-spawn**: <https://github.com/qbcore-framework/qb-spawn>
- **qb-apartments**: <https://github.com/qbcore-framework/qb-apartments>
- **qb-garages**: <https://github.com/qbcore-framework/qb-garages>

### Inventory & Items

- **qb-inventory**: <https://github.com/qbcore-framework/qb-inventory>
- **qb-shops**: <https://github.com/qbcore-framework/qb-shops>
- **qb-weapons**: <https://github.com/qbcore-framework/qb-weapons>
- **qb-items**: <https://docs.qbcore.org/qbcore-documentation/qbcore-resources/qb-items>

### Jobs

- **qb-police**: <https://github.com/qbcore-framework/qb-policejob>
- **qb-ambulancejob**: <https://github.com/qbcore-framework/qb-ambulancejob>
- **qb-mechanicjob**: <https://github.com/qbcore-framework/qb-mechanicjob>
- **qb-management**: <https://github.com/qbcore-framework/qb-management>
- **qb-bossmenu**: <https://github.com/qbcore-framework/qb-bossmenu>

### Vehicles

- **qb-vehicleshop**: <https://github.com/qbcore-framework/qb-vehicleshop>
- **qb-vehiclekeys**: <https://github.com/qbcore-framework/qb-vehiclekeys>
- **qb-vehiclefailure**: <https://github.com/qbcore-framework/qb-vehiclefailure>
- **qb-tunerchip**: <https://github.com/qbcore-framework/qb-tunerchip>
- **qb-customs**: <https://github.com/qbcore-framework/qb-customs>

### Economy & Banking

- **qb-banking**: <https://github.com/qbcore-framework/qb-banking>
- **qb-atms**: <https://github.com/qbcore-framework/qb-atms>
- **qb-crypto**: <https://github.com/qbcore-framework/qb-crypto>
- **qb-pawnshop**: <https://github.com/qbcore-framework/qb-pawnshop>

### Phone & Communication

- **qb-phone**: <https://github.com/qbcore-framework/qb-phone>
- **qb-dispatch**: <https://github.com/qbcore-framework/qb-dispatch>
- **qb-radio**: <https://github.com/qbcore-framework/qb-radio>

### Interaction Systems

- **qb-target**: <https://github.com/qbcore-framework/qb-target>
- **qb-input**: <https://github.com/qbcore-framework/qb-input>
- **qb-menu**: <https://github.com/qbcore-framework/qb-menu>
- **qb-skillbar**: <https://github.com/qbcore-framework/qb-skillbar>

### Utilities

- **qb-adminmenu**: <https://github.com/qbcore-framework/qb-adminmenu>
- **qb-logs**: <https://github.com/qbcore-framework/qb-logs>
- **qb-scoreboard**: <https://github.com/qbcore-framework/qb-scoreboard>
- **qb-hud**: <https://github.com/qbcore-framework/qb-hud>
- **qb-loading**: <https://github.com/qbcore-framework/qb-loading>

### Activities & Entertainment

- **qb-drugs**: <https://github.com/qbcore-framework/qb-drugs>
- **qb-lapraces**: <https://github.com/qbcore-framework/qb-lapraces>
- **qb-casino**: <https://github.com/qbcore-framework/qb-casino>
- **qb-diving**: <https://github.com/qbcore-framework/qb-diving>
- **qb-hunting**: <https://github.com/qbcore-framework/qb-hunting>
- **qb-houserobbery**: <https://github.com/qbcore-framework/qb-houserobbery>
- **qb-storerobbery**: <https://github.com/qbcore-framework/qb-storerobbery>

### Player Systems

- **qb-weathersync**: <https://github.com/qbcore-framework/qb-weathersync>
- **qb-smallresources**: <https://github.com/qbcore-framework/qb-smallresources>
- **qb-clothing**: <https://github.com/qbcore-framework/qb-clothing>
- **qb-tattooshop**: <https://github.com/qbcore-framework/qb-tattooshop>
- **qb-vehiclesales**: <https://github.com/qbcore-framework/qb-vehiclesales>

## Community Resources

- **Discord**: <https://discord.gg/qbcore>
- **GitHub Organization**: <https://github.com/qbcore-framework>
- **Documentation**: <https://docs.qbcore.org>

## Related Documentation

### FiveM Documentation

- **Natives Reference**: <https://docs.fivem.net/natives/>
- **Scripting Reference**: <https://docs.fivem.net/docs/scripting-reference/>
- **Resource Manifest**: <https://docs.fivem.net/docs/scripting-reference/resource-manifest/resource-manifest/>

### Database (oxmysql)

- **oxmysql Documentation**: <https://overextended.dev/oxmysql>

### UI Library (ox_lib)

- **ox_lib Documentation**: <https://overextended.dev/ox_lib>

### Target System

- **qb-target Documentation**: <https://docs.qbcore.org/qbcore-documentation/qbcore-resources/qb-target>

## Quick Access

When helping developers, reference these pages for:

- **Core Object**: <https://docs.qbcore.org/qbcore-documentation/qb-core/core-object>
- **Client Functions**: <https://docs.qbcore.org/qbcore-documentation/qb-core/client-function-reference>
- **Server Functions**: <https://docs.qbcore.org/qbcore-documentation/qb-core/server-function-reference>
- **Player Data**: <https://docs.qbcore.org/qbcore-documentation/qb-core/player-data>
- **Script Optimization**: <https://docs.qbcore.org/qbcore-documentation/guides/script-optimization>
- **Commands**: <https://docs.qbcore.org/qbcore-documentation/qb-core/commands>

## Installation & Setup

- **QBCore Installation Guide**: <https://docs.qbcore.org/qbcore-documentation/guides/qbcore-installation>
- **Database Setup**: <https://docs.qbcore.org/qbcore-documentation/guides/database-setup>
- **Server Configuration**: <https://docs.qbcore.org/qbcore-documentation/guides/server-configuration>
